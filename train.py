...
def train(opt):
    ...
    while(True):
        ...
        if 'CTC' in opt.Prediction:
            ...
            pad_mask = None
            # print(image.shape)
            preds, preds2, preds3 = model(image, pad_mask) 
            #print(text.size())
            loss1 = loss_function(preds.permute(0,2,1), text).mean(dim=1).mean()
            loss2 = loss_function(preds2.permute(0,2,1), text).mean(dim=1).mean()
            loss3 = loss_function(preds3.permute(0,2,1), text).mean(dim=1).mean()
            cost = (loss1 + loss2 + loss3) / 3.0
