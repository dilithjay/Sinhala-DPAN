...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, num_hiddens, dropout=0.1, max_len=1000):
        ...
        self.P = torch.zeros((32, max_len, num_hiddens))
    ...
    def forward(self, X):
        ...
        #print(X.size())
        X = X + self.P[:X.shape[0], :X.shape[1], :].to(X.device)
...
class BLMTransformer(nn.Module):
    ...
    def __init__(self, cfg, n_layers=4):
        ...
        self.maxlen = cfg.max_vocab_size

    def forward(self, g1, mask):
        blmMask = torch.zeros((self.maxlen, self.maxlen), device='cuda')
...
class Parallel_Attention2(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        self.soft = nn.Softmax(dim=1)
        self.cfg_dim = cfg.dim

    def forward(self, q,k,v, mask=None):
        x = torch.bmm(self.soft(torch.bmm(q, k.transpose(1,2)) / np.sqrt(self.cfg_dim)), v)   # b*512*94
        ##print(x.size())
        #bert_out = self.activ_fn(self.drop(self.atten_w1(bert_out)))
        #atten_w = self.soft(self.atten_w2(bert_out))                  # b*200*94
        #x = torch.bmm(origin_I.transpose(1,2), atten_w)               # b*512*94
...
class VSFD(nn.Module):
    def __init__(self, cfg, in_channels=512, pvam_ch=512, char_num=42):
        super(VSFD, self).__init__()
        self.char_num = char_num
        self.fc0 = nn.Linear(
            in_features=cfg.dim * 2, out_features=cfg.dim)
        self.fc1 = nn.Linear(
            in_features=cfg.dim, out_features=char_num)
        self.drop = nn.Dropout(0.1)
...
class Bert_Ocr(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.transformer = Transformer(cfg, cfg.attention_layers_PVAM)
        self.BLMtransformer = BLMTransformer(cfg, cfg.attention_layers_BLM)
...
#         self.attention = MultiHeadAttention(d_model=cfg.dim, max_vocab_size=cfg.max_vocab_size)
    ...
    def forward(self, encoder_feature, mask=None):
        ...
        ##print(glimpses.size())
        BLMout = self.softm(self.BLMtransformer(glimpses.detach(), mask))
        ...
        #print(vis_pred.size(), blm_pred.size(), out.size())
...
class Config(object):
    ...
    attention_layers_PVAM = 3                   # the layers of transformer PVAM
    attention_layers_BLM = 4                   # the layers of transformer BLM
    n_heads = 8
